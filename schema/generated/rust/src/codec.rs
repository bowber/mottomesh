...
impl Encode for ClientMessage {
    fn encode<W: Write>(&self, w: &mut W) -> IoResult<()> {
        match self {
            ...
            Self::Request { subject, payload, timeout_ms, request_id } => {
                4u8.encode(w)?;
                subject.encode(w)?;
                payload.encode(w)?;
                timeout_ms.encode(w)?;
...
impl Decode for ClientMessage {
    fn decode<R: Read>(r: &mut R) -> IoResult<Self> {
        ...
        match tag {
            ...
            5 => Ok(Self::Ping),
            _ => Err(IoError::new(ErrorKind::InvalidData, format!("Unknown ClientMessage tag: {}", tag))),
        }
    }
}
...
impl Encode for ServerMessage {
    fn encode<W: Write>(&self, w: &mut W) -> IoResult<()> {
        match self {
            ...
            Self::Message { subscription_id, subject, payload } => {
                4u8.encode(w)?;
...
impl Decode for ServerMessage {
    fn decode<R: Read>(r: &mut R) -> IoResult<Self> {
        ...
        match tag {
            ...
            8 => Ok(Self::Pong),
            _ => Err(IoError::new(ErrorKind::InvalidData, format!("Unknown ServerMessage tag: {}", tag))),
        }
    }
}
...
impl Decode for SchemaRouter {
    fn decode<R: Read>(r: &mut R) -> IoResult<Self> {
        ...
        match tag {
            ...
            _ => Err(IoError::new(ErrorKind::InvalidData, format!("Unknown SchemaRouter tag: {}", tag))),
        }
    }
}
